// Use API URL from config.js (with fallback)
const API_URL = (typeof CONFIG !== 'undefined' && CONFIG.API_URL) || window.API_URL || 'https://ozx9x2csyg.execute-api.ap-south-1.amazonaws.com/prod';
let accessToken = '';
let currentUser = {};
let inventoryData = [];
let udhaarData = [];
...
function showAuthScreen() {
    ...
    // Clear shop name to prevent showing old name
...
async function register() {
    ...
    try {
        ...
        if (response.ok) {
            alert('✅ Registration successful! Your account is ready. Please login now.');
            showAuthTab('login');
            // Pre-fill username
        ...
        if (response.ok && data.idToken) {
            ...
            try {
                ...
                currentUser = { 
                    ...
                    phone: payload.phone_number || payload['custom:phone'] || ''
                };
            } catch (e) {
                console.error('Error decoding token:', e);
                // Fallback
                currentUser = { username, shopName: username, email: '' };
            }
...
async function generateForecast() {
    ...
    console.log('Requesting forecast for', days, 'days');
    ...
    try {
        console.log('Generating forecast for:', itemIdArray, 'days:', days);
        
        // Use the correct endpoint: POST /forecast with body
        ...
        console.log('Number of forecasts:', data.forecasts?.length);
        
        // Add item names to forecast data
        if (data.forecasts) {
            data.forecasts.forEach((forecast, index) => {
                forecast.itemName = itemNames[index] || forecast.itemId;
                console.log(`Forecast for ${forecast.itemName}: ${forecast.forecast?.length} days`);
            });
        }
...
function displayForecast(forecasts) {
    ...
    console.log('Displaying forecast for', days, 'days');
    ...
    container.innerHTML = forecasts.map(forecast => {
        ...
        console.log(`Item ${forecast.itemName}: ${forecastDays.length} days in forecast`);
        
        return `
        <div class="forecast-item">
            <h4>${forecast.itemName || 'Unknown Item'}</h4>
            <p style="color: #666; font-size: 0.9rem;">Item ID: ${forecast.itemId} | Showing ${Math.min(days, forecastDays.length)} of ${forecastDays.length} days</p>
            <div class="forecast-chart">
                ${forecastDays.slice(0, days).map(day => `
                    <div class="forecast-day">
                        ...
                        ${day.isFestival ? `<div class="festival-badge">🎉 ${day.festivalName}</div>` : ''}
                        ${day.isWeekend ? `<div class="weekend-badge">📅</div>` : ''}
                    </div>
                `).join('')}
            </div>
...
async function loadSalesData() {
    try {
        const period = document.getElementById('salesPeriodFilter')?.value || '30d';
        const paymentMethod = document.getElementById('salesPaymentFilter')?.value || '';
        
        let url = `${API_URL}/sales?period=${period}`;
        if (paymentMethod) {
...
function displaySalesTable() {
    ...
    tbody.innerHTML = salesData.map(sale => {
        const date = new Date(sale.saleDate).toLocaleString('en-IN', {
            day: '2-digit',
            month: 'short',
            hour: '2-digit',
            minute: '2-digit'
        ...
        return `
            <tr>
                ...
                <td><span class="badge badge-${sale.paymentMethod}">${sale.paymentMethod}</span></td>
                <td>${sale.customerName || '-'}</td>
                <td>
                    <button class="btn-icon" onclick="viewSaleDetails('${sale.saleId}')" title="View Details">👁️</button>
                    <button class="btn-icon" onclick="deleteSale('${sale.saleId}')" title="Delete">🗑️</button>
                </td>
            </tr>
        `;
    }).join('');
...
async function loadSalesAnalytics() {
    try {
        ...
        // Update stats
        document.getElementById('todaySales').textContent = `₹${parseFloat(analytics.todayRevenue || 0).toFixed(0)}`;
        document.getElementById('weekSales').textContent = `₹${parseFloat(analytics.weekRevenue || 0).toFixed(0)}`;
        document.getElementById('monthSales').textContent = `₹${parseFloat(analytics.totalRevenue || 0).toFixed(0)}`;
        document.getElementById('avgOrderValue').textContent = `₹${parseFloat(analytics.averageOrderValue || 0).toFixed(0)}`;
...
function showNewSaleModal() {
    try {
        ...
        const customerNameEl = document.getElementById('saleCustomerName');
        const notesEl = document.getElementById('saleNotes');
        
        if (!saleItemsEl) {
            console.error('saleItems element not found');
            alert('Error: Sale form not found. Please refresh the page.');
            return;
        }
        ...
        // Open modal
        console.log('Opening newSaleModal');
        openModal('newSaleModal');
    } catch (error) {
        console.error('Error in showNewSaleModal:', error);
        alert('Error opening sale form: ' + error.message);
    }
}

function addSaleItem() {
    const container = document.getElementById('saleItems');
    const itemId = `saleItem${saleItemCounter++}`;
    
    const itemHtml = `
        <div class="sale-item-row" id="${itemId}" style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 40px; gap: 10px; margin-bottom: 10px; align-items: end;">
            <div>
                <label>Item</label>
                <select class="sale-item-select" onchange="updateSaleItemPrice('${itemId}')">
            ...
            <div>
                ...
                <input type="number" class="sale-item-qty" min="1" value="1" onchange="calculateSaleItemTotal('${itemId}')">
            </div>
            <div>
                <label>Price (₹)</label>
                <input type="number" class="sale-item-price" step="0.01" min="0" value="0" onchange="calculateSaleItemTotal('${itemId}')">
            </div>
            <div>
                <label>Total (₹)</label>
                <input type="number" class="sale-item-total" readonly value="0">
            </div>
            <button class="btn-icon" onclick="removeSaleItem('${itemId}')" title="Remove" style="margin-top: 20px;">❌</button>
        </div>
    `;
...
function updateSaleItemPrice(itemId) {
    ...
    calculateSaleItemTotal(itemId);
}

function calculateSaleItemTotal(itemId) {
    const row = document.getElementById(itemId);
...
function updateSaleTotal() {
    ...
    rows.forEach(row => {
        ...
        if (select.value) {
            itemCount++;
            const total = parseFloat(row.querySelector('.sale-item-total').value) || 0;
            totalAmount += total;
        }
    });
    
    document.getElementById('saleItemCount').textContent = itemCount;
...
async function recordSale() {
    try {
        ...
        for (const row of rows) {
            ...
            if (qty <= 0 || price <= 0) {
                showToast('Please enter valid quantity and price for all items', 'error');
                return;
            }
        ...
        const saleData = {
            ...
            customerName: document.getElementById('saleCustomerName').value || null,
...
function showQuickSale() {
    try {
        const section = document.getElementById('quickSaleSection');
        if (!section) {
            console.error('quickSaleSection not found');
            alert('Error: Quick Sale section not found. Please refresh the page.');
            return;
        }
        ...
        if (section.style.display === 'block') {
            console.log('Populating quick sale buttons');
            populateQuickSaleButtons();
        }
    } catch (error) {
        console.error('Error in showQuickSale:', error);
        alert('Error opening Quick Sale: ' + error.message);
    }
}
...
function populateQuickSaleButtons() {
    ...
    // Get top 12 most common items from inventory
    const commonItems = inventoryData
        .filter(item => item.quantity > 0)
        .slice(0, 12);
    
    if (commonItems.length === 0) {
        container.innerHTML = '<p>No items in inventory. Please add items first.</p>';
        return;
    }
    
    container.innerHTML = commonItems.map(item => `
        <button class="quick-sale-btn" onclick="addToQuickCart('${item.itemId}', '${item.name}', ${item.sellingPrice})">
            <div style="font-size: 2em; margin-bottom: 5px;">${getCategoryEmoji(item.category)}</div>
            <div style="font-weight: bold;">${item.name}</div>
            <div style="color: #666;">₹${item.sellingPrice}</div>
        </button>
    `).join('');
}
...
function addToQuickCart(itemId, name, price) {
    // Check if item already in cart
    const existing = quickSaleCart.find(item => item.itemId === itemId);
...
function updateQuickSaleDisplay() {
    const cartContainer = document.getElementById('quickSaleCart');
    ...
    cartContainer.innerHTML = quickSaleCart.map((item, index) => `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid #eee;">
            <div>
                <strong>${item.name}</strong><br>
                <span style="color: #666;">₹${item.unitPrice} × ${item.quantity}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 10px;">
                <button onclick="changeQuickCartQty(${index}, -1)" style="padding: 5px 10px; background: #ff6b6b; color: white; border: none; border-radius: 4px; cursor: pointer;">-</button>
                <span style="font-weight: bold;">${item.quantity}</span>
                <button onclick="changeQuickCartQty(${index}, 1)" style="padding: 5px 10px; background: #51cf66; color: white; border: none; border-radius: 4px; cursor: pointer;">+</button>
                <button onclick="removeFromQuickCart(${index})" style="padding: 5px 10px; background: #868e96; color: white; border: none; border-radius: 4px; cursor: pointer;">🗑️</button>
            </div>
        </div>
    `).join('');
    
    const total = quickSaleCart.reduce((sum, item) => sum + item.total, 0);
    totalContainer.textContent = `₹${total.toFixed(2)}`;
}

function changeQuickCartQty(index, change) {
    quickSaleCart[index].quantity += change;
    
    if (quickSaleCart[index].quantity <= 0) {
        quickSaleCart.splice(index, 1);
    } else {
        quickSaleCart[index].total = quickSaleCart[index].quantity * quickSaleCart[index].unitPrice;
    }
...
function clearQuickSale() {
    if (quickSaleCart.length > 0 && !confirm('Clear all items from cart?')) {
        return;
    }
    quickSaleCart = [];
    updateQuickSaleDisplay();
...
async function completeQuickSale() {
    ...
    const totalAmount = quickSaleCart.reduce((sum, item) => sum + item.total, 0);
    
    // Ask for payment method
    const paymentMethod = prompt('Payment method?\n1 = Cash\n2 = UPI\n3 = Card\n4 = Udhaar', '1');
    const methodMap = {'1': 'cash', '2': 'upi', '3': 'card', '4': 'udhaar'};
...
function startWebSpeechRecognition() {
    ...
    recognition.onresult = async (event) => {
        ...
        // Process the query
        await processVoiceQuery(transcript, language);
    };
...
async function processVoiceQuery(text, language) {
    const status = document.getElementById('voiceStatus');
    ...
    try {
        ...
        if (intent.type === 'query') {
            response = await handleVoiceQuery(intent);
        } else if (intent.type === 'simulation') {
            response = await handleVoiceSimulation(intent);
        } else {
            response = { text: 'मुझे समझ नहीं आया। कृपया फिर से पूछें। (I didn\'t understand. Please ask again.)' };
        }
        
        // Show response
        document.getElementById('voiceResponse').style.display = 'block';
        document.getElementById('voiceResponseText').textContent = response.text;
        ...
        status.textContent = 'Done! Ask another question or close.';
        status.style.color = '#4CAF50';
        
    } catch (error) {
        console.error('Voice processing error:', error);
        showToast('Failed to process voice query', 'error');
...
function classifyIntent(text) {
    ...
    // Simulation patterns
    if (textLower.includes('what if') || textLower.includes('अगर') || textLower.includes('यदि')) {
        if (textLower.includes('udhaar') || textLower.includes('उधार') || textLower.includes('credit')) {
            return { type: 'simulation', subtype: 'udhaar', text };
        } else if (textLower.includes('price') || textLower.includes('कीमत') || textLower.includes('dam')) {
            return { type: 'simulation', subtype: 'price', text };
        }
    }
    
    // Query patterns
    if (textLower.includes('udhaar') || textLower.includes('उधार')) {
        return { type: 'query', subtype: 'udhaar_status', text };
    } else if (textLower.includes('stock') || textLower.includes('माल') || textLower.includes('low') || textLower.includes('कम')) {
        return { type: 'query', subtype: 'low_stock', text };
    } else if (textLower.includes('sales') || textLower.includes('बिक्री') || textLower.includes('sell') || textLower.includes('बेच')) {
        return { type: 'query', subtype: 'sales', text };
    }
    
    return { type: 'unknown', text };
}

async function handleVoiceQuery(intent) {
    if (intent.subtype === 'udhaar_status') {
        // Get actual udhaar data
        const total = udhaarData.reduce((sum, r) => sum + (r.outstandingAmount || 0), 0);
        const overdue = udhaarData.filter(r => r.status === 'OVERDUE').reduce((sum, r) => sum + (r.outstandingAmount || 0), 0);
        
        return {
            text: `आपका कुल उधार ₹${total.toFixed(0)} है। ${overdue > 0 ? `₹${overdue.toFixed(0)} 30 दिन से ज्यादा हो गए हैं।` : 'सभी उधार समय पर हैं।'}`
        };
    } else if (intent.subtype === 'low_stock') {
        const lowItems = inventoryData.filter(item => item.quantity < item.minStockLevel);
        if (lowItems.length === 0) {
            return { text: 'सभी माल पर्याप्त है। (All items are sufficient.)' };
        }
        const itemNames = lowItems.slice(0, 3).map(i => i.name).join(', ');
        return { text: `${lowItems.length} चीजें कम हैं: ${itemNames}` };
    } else if (intent.subtype === 'sales') {
        return { text: 'आज की बिक्री की जानकारी लोड हो रही है। (Loading today\'s sales information.)' };
    }
    
    return { text: 'जानकारी उपलब्ध नहीं है। (Information not available.)' };
}

async function handleVoiceSimulation(intent) {
    if (intent.subtype === 'udhaar') {
        return {
            text: 'अगर आप ₹5000 और उधार देते हैं, तो 10 दिन में नकदी की कमी हो सकती है। कम मार्जिन वाली चीजों पर उधार सीमित करें। (If you give ₹5000 more udhaar, cash shortage risk in 10 days. Limit udhaar on low-margin items.)'
        };
    } else if (intent.subtype === 'price') {
        return {
            text: 'कीमत बढ़ाने से बिक्री 7% कम हो सकती है, लेकिन मुनाफा बढ़ेगा। (Price increase may reduce sales by 7%, but profit will increase.)'
        };
    }
    
    return { text: 'सिमुलेशन उपलब्ध नहीं है। (Simulation not available.)' };
}

async function processVoiceInput(audioBlob) {
    // This would send to backend for transcription
    // For now, show a message
    showToast('Voice processing not yet connected to backend', 'info');
    resetVoiceButton();
}
